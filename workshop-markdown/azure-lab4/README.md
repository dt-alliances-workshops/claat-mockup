...
2. Trigger a CPU Problem
    ...
    * From the Azure shell, SSH to the monolith virtual machine with this command:
    ...
    * Using the IP address for the MONOLITH host, run this command:
        ...
        **ℹ️ NOTE: Ask instructor for password**

    * Once SSH session is established, copy all these lines and run them:
...
4. Stop the CPU problem
    * To stop the problem, you need to `kill` the processes.  To do this:
        - Run this command to get the process IDs:

            ```
            ps -ef | grep yes
            ```

    * For each process, copy the process ID and run 
